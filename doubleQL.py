...
if __name__ == '__main__':
    ...
    ALPHA = 0.1
    GAMMA = 0.9 # rewards are certain, so no discounting needed   
    EPS = 1.0
    ...
    for i in range(numGames):
        ...
        while not done:
            ...
            # seperate random number which determines which Q functions we're going to update.
            rand = np.random.random()
            # loop for alternating between the Q functions
            if rand <= 0.5:
        ...
        # strategy to deal with explore and exploit delemma.
